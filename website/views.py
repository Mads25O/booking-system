...
from . import db
from .models import Patient, Bookings
...
def patient_home():
    ...
    if request.method != 'POST':
        return render_template('patient-side.html', user=current_user, user_type=user_type)
...
def booking():
    ...
    booking_entry = Bookings.query.filter_by(user_id=user_id).first()
    ...
    booking_entry = Bookings(user_id=user_id, bookings=[new_booking])
